...
# ============================================================
# Lightweight graph helpers (placeholder graph + attributes)
# ============================================================
def build_dummy_weather_graph(
    ...
    edge_attr_dim: int = 2,
    data_attr_dim: int = 2,
    hidden_attr_dim: int = 2,
) -> HeteroData:
    """
    Build a minimal heterogeneous graph with the structure expected by the
    encode-process-decode pipeline:
    data -> hidden, hidden -> hidden, hidden -> data

    The contents are dummy (random) but shapes match what the model expects.
    """
    graph = HeteroData()

    # Node attributes (e.g., lat/lon encoded as sin/cos). Here: random.
    graph["data"].x = torch.randn(num_data_nodes, data_attr_dim)
    graph["hidden"].x = torch.randn(num_hidden_nodes, hidden_attr_dim)

    # Simple edge wiring: connect each src node to a handful of dst nodes.
    def dense_edges(n_src: int, n_dst: int, fanout: int = 4) -> torch.Tensor:
    ...
    # Edge attributes (lengths, directions, etc.). Dummy values with requested dim.
    for key in [
        ("data", "to", "hidden"),
        ("hidden", "to", "hidden"),
        ("hidden", "to", "data"),
...
# ============================================================
# Core model blocks (Hydra classes reimplemented without sharding)
# ============================================================
...
class TrainableFeatures(nn.Module):
    """
    Trainable node features stored as a parameter and concatenated to node
    attributes/features at runtime.
    ...
    feature_dim : int
        Dimension of the learnable features.

    Returns
    -------
    torch.Tensor
        Trainable features of shape [num_nodes, feature_dim].
    """

    def __init__(self, num_nodes: int, feature_dim: int):
        super().__init__()
        self.num_nodes = num_nodes
        self.feature_dim = feature_dim
        param = nn.Parameter(torch.zeros(num_nodes, feature_dim))
...
def run_message_op(
    ...
    """
    Call a MessagePassing op while gracefully ignoring edge_attr if the op
    doesn't accept it. Falls back to edge_weight if available.
    """
...
class WeatherForwardMapper(nn.Module):
    """
    Slimmed-down GNNForwardMapper: embeds source/destination features and
    applies a message-passing update from src -> dst.
    ...
    message_op : MessagePassing
        PyG message passing layer (e.g., SAGEConv) applied to (src, dst).
...
class WeatherProcessor(nn.Module):
    """
    Slim GNNProcessor with a single message-passing update on hidden->hidden
    edges plus residual and LayerNorm.
...
class WeatherBackwardMapper(nn.Module):
    """
    Slimmed-down GNNBackwardMapper that embeds hidden/data nodes, performs
    message passing hidden -> data, then projects to outputs.
...
class WeatherEncProcDec(nn.Module):
    """
    End-to-end encode-process-decode model mirroring AnemoiModelEncProcDec,
    stripped of sharding/distributed mechanics.
    ...
    in_channels_data : int, default 8
        Number of data feature channels.
    out_channels : int, default 8
        Number of output channels.
    data_attr_dim : int, default 0
        Attribute dimensions appended to data nodes.
    hidden_attr_dim : int, default 0
        Attribute dimensions appended to hidden nodes.
    trainable_data_dim : int, default 0
        Learnable feature dimensions per data node (concatenated to inputs).
    trainable_hidden_dim : int, default 0
        Learnable feature dimensions per hidden node (concatenated to hidden features).
    ...
    def __init__(
        ...
        processor: WeatherProcessor,
        decoder: WeatherBackwardMapper,
        in_channels_data: int = 8,
        out_channels: int = 8,
        data_attr_dim: int = 0,
        hidden_attr_dim: int = 0,
        trainable_data_dim: int = 0,
        trainable_hidden_dim: int = 0,
    ):
        super().__init__()
        self.graph = graph
        ...
        self.in_channels_data = in_channels_data
        self.out_channels = out_channels
        self.data_attr_dim = data_attr_dim
        self.hidden_attr_dim = hidden_attr_dim
        self.trainable_data_dim = trainable_data_dim
        self.trainable_hidden_dim = trainable_hidden_dim
        self.trainable_data_feats = (
            TrainableFeatures(graph["data"].num_nodes, trainable_data_dim) if trainable_data_dim > 0 else None
        )
        self.trainable_hidden_feats = (
            TrainableFeatures(graph["hidden"].num_nodes, trainable_hidden_dim) if trainable_hidden_dim > 0 else None
        )

    def forward(self, graph: HeteroData) -> torch.Tensor:
        """
        graph: HeteroData (potentially batched/disconnected) containing
               node types 'data' and 'hidden' and edges:
               ('data','to','hidden'), ('hidden','to','hidden'), ('hidden','to','data')
        returns: tensor shaped [total_data_nodes, out_channels]
        """
        ...
        hidden_with_attr = hidden_latent
        hidden_with_attr = torch.cat([hidden_with_attr, graph["hidden"].x], dim=-1)
        if self.trainable_hidden_feats is not None:
            hidden_with_attr = torch.cat(
                [hidden_with_attr, self.trainable_hidden_feats(graph["hidden"].num_nodes).to(hidden_with_attr.device)],
                dim=-1,
            )
        data_out = self.decoder(
            x_src=hidden_with_attr,
            x_dst=data_feats,
...
class DummyWeatherDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> HeteroData:
        ...
        graph["data"].x = torch.randn(self.num_data_nodes, self.in_channels)
        graph["hidden"].x = torch.zeros(graph["hidden"].num_nodes, 0)
        graph["data"].y = torch.randn(self.num_data_nodes, self.out_channels)
        return graph
...
def experiment_factory() -> Experiment:
    ...
    # Embeddings mimic the mapper MLPs (data/hidden plus positional attributes)
    encoder = WeatherForwardMapper(
    ...
    decoder = WeatherBackwardMapper(
        ...
        message_op=SAGEConv((hidden_dim, hidden_dim), hidden_dim),
        out_linear=nn.Linear(hidden_dim, out_channels),
    )
...
def main():
    exp_cfg = experiment_factory.as_buildable()
    experiment = exp_cfg()
    experiment.run()
