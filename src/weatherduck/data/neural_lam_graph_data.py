...
def _ensure_tuple_of_tensors(value: object) -> Tuple[Tensor, ...]:
    """Normalize a tensor or iterable of tensors into a tuple.

    Parameters
    ----------
    value : object
        Tensor or iterable of tensors.
...
class GraphEdgesAndFeatures:
    ...
    g2m_edge_index : Tensor
        Adjacency list for grid-to-mesh edges (shape ``[2, E_g2m]``).
    m2g_edge_index : Tensor
        Adjacency list for mesh-to-grid edges (shape ``[2, E_m2g]``).
    m2m_edge_index : Tuple[Tensor, ...]
        Adjacency list(s) for intra-level mesh edges.
    mesh_up_edge_index : Tuple[Tensor, ...]
        Edge indices connecting level ``l`` to ``l+1`` (hierarchical only).
    mesh_down_edge_index : Tuple[Tensor, ...]
        Edge indices connecting level ``l+1`` back to ``l`` (hierarchical only).
    g2m_features : Tensor
        Feature matrix for grid-to-mesh edges (shape ``[E_g2m, F_g2m]``).
    m2g_features : Tensor
        Feature matrix for mesh-to-grid edges (shape ``[E_m2g, F_m2g]``).
    m2m_features : Tuple[Tensor, ...]
        Feature matrices for intra-level mesh edges.
    mesh_up_features : Tuple[Tensor, ...]
        Feature matrices for upward inter-level edges.
    mesh_down_features : Tuple[Tensor, ...]
        Feature matrices for downward inter-level edges.
    mesh_static_features : Tuple[Tensor, ...]
    ...
    def __post_init__(self) -> None:
        """Validate tensor shapes and hierarchy consistency.
        ...
        if self.hierarchical:
            if num_levels < 2:
                raise ValueError("Hierarchical graphs require at least two mesh levels")
    ...
    def num_levels(self) -> int:
        """Return number of mesh levels represented by this graph."""
    ...
    def as_batch_dict(self) -> Dict[str, object]:
        ...
        -------
        Dict[str, object]
            Graph payload matching neural-lam batch expectations.
        """
...
def _to_tuple(values: Iterable[int]) -> Tuple[int, ...]:
    """Ensure iterable of ints is a tuple.
...
class GraphSizes:
    ...
    def __post_init__(self) -> None:
        """Validate size metadata consistency.
        ...
        object.__setattr__(self, "m2m_feature_dims", _to_tuple(self.m2m_feature_dims))
    ...
    @property
    def num_levels(self) -> int:
        """Return number of mesh levels represented by this metadata."""
...
def load_graph(
    ...
    if not graph_dir.exists():
        raise FileNotFoundError(f"Graph directory {graph_dir} could not be found.")

    def loads_file(filename: str) -> Tensor:
        return torch.load(graph_dir / filename, map_location=device, weights_only=True)

    m2m_edge_index_list = [tensor.clone() for tensor in loads_file("m2m_edge_index.pt")]
    ...
    m2m_features_list = [tensor.clone() for tensor in loads_file("m2m_features.pt")]
    ...
    if len(m2m_features_list) != n_levels:
        raise ValueError("Inconsistent number of levels in mesh for edge features.")
    if len(mesh_static_features_list) != n_levels:
        raise ValueError("Inconsistent number of levels in mesh for node features.")

    hierarchical_kwargs: Dict[str, Tuple[Tensor, ...]] = {}
    if hierarchical:
        ...
        mesh_down_features_list = [
            edge_features / longest_edge for edge_features in mesh_down_features_list
        ]
    ...
    return GraphEdgesAndFeatures(**base_kwargs, **hierarchical_kwargs)  # type: ignore


def build_graph_sizes(
    graph_edges_and_features: GraphEdgesAndFeatures,
) -> GraphSizes:
    """Build graph size metadata from a GraphEdgesAndFeatures instance.

    Parameters
    ----------
    graph_edges_and_features : GraphEdgesAndFeatures
        Graph adjacency and feature tensors.

    Returns
    -------
    GraphSizes
        Size metadata derived from the graph.
    """
    mesh_level_sizes = [
    ...
    hierarchical_kwargs: Dict[str, Tuple[int, ...]] = {}
    if graph_edges_and_features.hierarchical:
        hierarchical_kwargs = {
            "mesh_up_feature_dims": tuple(
                tensor.shape[1] for tensor in graph_edges_and_features.mesh_up_features
            ),
